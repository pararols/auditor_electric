...
def align_solar(model_series: pd.Series, target_index: pd.DatetimeIndex) -> np.ndarray:
    ...
    """Mapeja la corba solar del model (2025) sobre qualsevol índex DatetimeIndex
...
def run_optimization(df_consum, prices, excedent_price, min_kwp_threshold=0.0):
    ...
    # Carregar corbes model (8760h) i alinear amb l'índex de consum
...
def render_cle_optimizer():
    ...
    if run_btn:
        ...
        if df_consum is None or df_consum.empty:
            ...
            st.error("No s'han trobat dades de consum suficients per al període seleccionat.")
        ...
        # Optimització SLSQP fixes (pestanya actual)
    ...
    tab_fix, tab_var = st.tabs(["📌 Coeficients Fixes (SLSQP)", "🔀 Coeficients Variables (Horaris)"])
    ...
    with tab_fix:
        ...
        if st.session_state.cle_results is not None:
            ...
            fig_donut1.update_layout(title_text="Desglossament Econòmic (€)", title_x=0.5,
            ...
            fig_donut2.update_layout(title_text="Destí Energia Generada (kWh)", title_x=0.5,
            ...
            fig_donut3.update_layout(title_text="Procedència del Consum (kWh)", title_x=0.5,
            ...
            df_display['Potència Sala Nova (kWp)'] = df_display['Potència Sala Nova (kWp)'].apply(

                lambda x: f"{x:.2f} kWp" if pd.notnull(x) else "")

            df_display['Coeficient Pavelló'] = df_display['Coeficient Pavelló'].apply(

                lambda x: f"{x:.6f}")

            df_display['Potència Pavelló (kWp)'] = df_display['Potència Pavelló (kWp)'].apply(
            ...
            df_display['Producció FV'] = df_display['Producció FV (kWh)'].apply(
            ...
            df_display['Autoconsum (Pavelló)'] = df_display['Autoconsum PAV (kWh)'].apply(
            ...
            df_display['Excedents Llençats'] = df_display['Excedents Llençats a la xarxa (kWh)'].apply(
            ...
            for m in range(1, 13):
                ...
                m_gen_sn = sum([list(filter(lambda x: x['Mes'] == m, r['Mensual']))[0]['Generació Assignada SN']

                                for r in detailed_results])

                m_gen_pav = sum([list(filter(lambda x: x['Mes'] == m, r['Mensual']))[0]['Generació Assignada PAV']
            ...
            fig1.add_trace(go.Bar(x=df_months['Mes'], y=df_months['Autoconsum PAV (kWh)'],
                                  ...
                                  name='Autoconsum (Pavelló)', marker_color='#2e7d32'))

            fig1.update_layout(barmode='stack',

                               title="Estructura de l'Energia Mensual per a l'Administració",
            ...
            fig3.add_trace(go.Bar(x=df_months['Mes'], y=exc_pav,
                                  ...
                                  name='Excedents No Aprofitats (Pavelló)', marker_color='#a5d6a7'))

            fig3.update_layout(barmode='stack',

                               title='Destí de la Generació Solar: Aprofitament vs Excedents',
