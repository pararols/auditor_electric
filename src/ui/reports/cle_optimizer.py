...
from src.ui.reports.cle_variable_coefs import render_cle_variable_optimizer

# ParÃ metres globals Subapp
TARGET_KWP = 78.198
PAVELLO_KWP = 127.2
TARGET_RATIO = TARGET_KWP / PAVELLO_KWP

def load_solar_curves():
    """Carrega les corbes de generaciÃ³ anual del PavellÃ³ (127.2 kWp) i Sala Nova (17.1 kWp).
    Retorna dos Series amb Ã­ndex Datetime (any 2025 fictici per a mapping).
    """
    ...
    # Crear Ã­ndex temporal 2025 (no traspÃ s) per a 8760 hores
...
def align_solar(model_series: pd.Series, target_index: pd.DatetimeIndex) -> np.ndarray:
    """Mapeja la corba solar del model (2025) sobre qualsevol Ã­ndex DatetimeIndex
    usant la clau (mes, dia, hora), ignorant l'any."""
...
def fetch_and_prep_consumption(start_date, end_date):
    """Carrega el consum horari per a un perÃ­ode de 1 any per als CUPS municipals."""
    # start_date, end_date sÃ³n objectes date o strings 'YYYY-MM-DD'
    ...
    # Generar Ã­ndex complet per a l'any seleccionat (8760-8784 hores depenent de traspÃ s)
    # Per simplificar a l'optimitzador, forÃ§arem 8760 punts basant-nos en el rang
    full_index = pd.date_range(start=f"{start_str} 00:00:00", end=f"{end_str} 23:00:00", freq='h')
    
    # Si Ã©s any de traspÃ s o hi ha desviaments horaris, re-calculem per tenir un any estÃ ndard per a la comparativa solar
    # (L'algorisme estÃ  dissenyat per a vectors de 8760)
...
def calculate_tariffs(full_index, p1=0.22, p2=0.147, p3=0.11):
    ...
    # LÃ²gica senzilla per blocs (3.0TD tÃ© 6 periodes, 2.0TD en tÃ© 3). 
    # Suposarem la 2.0TD o una simplificaciÃ³: 
...
def evaluate_coefficients(coefs, cups_names, df_consum, gen_pavello, gen_salanova, prices, excedent_price):
    """
    Simula la facturaciÃ³ anual neta dels CUPS donats uns coeficients del pavellÃ³.
    ...
    for i, cups in enumerate(cups_names):
        ...
        # Quota fixada a configuraciÃ³ per Sala Nova (si hi estÃ  present)
        ...
        # CÃ lcul mes a mes per aplicar el lÃ­mit RD 244/2019
        for m in range(1, 13):
            mask = (months == m)
            ...
            # Topall legal compensaciÃ³: "El terme d'energia no pot ser inferior a 0 abans d'impostos"
            ...
            if v_excedents <= c_sol_import:
                ...
                # Hem topat. La part d'energia que hem cobrat virtualment Ã©s c_sol_import / excedent_price
            ...
            mensual_stats.append({
                ...
                'GeneraciÃ³ Assignada': np.sum(gen_total[mask]),
                'GeneraciÃ³ Assignada SN': np.sum(gen_sn[mask]),
                'GeneraciÃ³ Assignada PAV': np.sum(gen_pav[mask]),
        ...
        results_by_cups.append({
            ...
            'PotÃ¨ncia Sala Nova (kWp)': coef_sn * 17.1,  # Based on 17.1 total
            'Coeficient PavellÃ³': coef_pav,
            'PotÃ¨ncia PavellÃ³ (kWp)': coef_pav * PAVELLO_KWP,
            'Consum Anual (kWh)': np.sum(consum),
            'ProducciÃ³ FV (kWh)': np.sum(gen_total),
...
def run_optimization(df_consum, prices, excedent_price, min_kwp_threshold=0.0):
    ...
    # Carregar corbes model (8760h) i alinear amb l'Ã­ndex de consum
    ...
    # LÃ­mits actius variables (podem forÃ§ar 0 si pertoca)
    active_bounds = [(0, 1) for _ in range(N)]
    
    # RestricciÃ³: suma de coeficients = TARGET_RATIO
    constraints = {'type': 'eq', 'fun': lambda c: np.sum(c) - TARGET_RATIO}
    
    # --- InterfÃ­cie de Temps Real ---
    st.markdown("#### ðŸ”„ ProgrÃ©s de l'OptimitzaciÃ³ (Temps Real)")
    ...
    with st.spinner("L'algoritme d'optimitzaciÃ³ iteratiu estÃ  determinant el repartiment mÃ xim... (aixÃ² pot trigar uns segons)"):
        opt_res = minimize(
            objective_function, 
            init_guess,
    ...
    if diff > 0:
        # Repartir el diff restant als que tenen el remainder mÃ©s gran
...
def render_cle_optimizer():
    # Estils d'impressiÃ³ CSS (Amagar barra lateral i botons)
    st.markdown("""
        <style>
        @media print {
    ...
    st.subheader("âš™ï¸ Optimitzador CLE: AssignaciÃ³ Repartiment PavellÃ³ (RD 244/2019)")
    st.markdown("""
        Aquesta subaplicaciÃ³ determina automÃ ticament els **coeficients fixes ideals anuals** per a la nova instalÂ·laciÃ³ **FV PavellÃ³ (127.2 kWp)**, amb l'objectiu de distribuir exclusivament la quota reservada per a l'Ajuntament (78.198 kWp).
        
        **Criteris aplicats segons normativa ESP:**
        * L'energia provinent de la instalÂ·laciÃ³ Sala Nova es tÃ© en compte prÃ¨viament per calcular consums romanents.
        * SimulaciÃ³ amb freqÃ¼Ã¨ncia **horÃ ria** utilitzant histÃ²rÃ¬cs de 2025 i extrapolaciÃ³ de les dues corbes FV.
        * El lÃ­mit de compensaciÃ³ d'excedents mensual s'aplica estrictament abans d'impostos.
        * Totes les xifres econÃ²miques estan finalment gravades amb Impost ElÃ¨ctric (5.11%) i IVA (21%).
    """)
    
    # --- GESTIÃ“ D'ESTAT (SESSION STATE) ---
    ...
    st.markdown("#### ParÃ metres d'AnÃ lisi i Opcions Legals")
    
    # 1. Trobar data lÃ­mit per defecte
    municipal_cups_ids = [k for k, v in CUPS_MAPPING.items() if not str(v).startswith("Part. Privat")]
    default_end_date = get_last_complete_day_all_cups(municipal_cups_ids)
    
    # 2. UI Selector de PerÃ­ode
    col0, col1, col2, col3, col4 = st.columns([2, 1, 1, 1, 1.5])
    with col0:
        current_end_date = st.date_input("Analitzar perÃ­ode de 1 any fins a:", value=default_end_date or pd.Timestamp.now().date())
        start_date = current_end_date - pd.Timedelta(days=364)
        st.info(f"Rang d'anÃ lisi: {start_date.strftime('%d/%m/%Y')} - {current_end_date.strftime('%d/%m/%Y')}")

    # Nova sub-opciÃ³ ben visible a dalt (Poda de <0.5 kWp)
    filter_micro = col4.checkbox("Excloure < 0.5 kWp", value=True, help="Simplifica la gestiÃ³ administrativa municipal")
    ...
    run_btn = col_run.button("Executar Motor d'OptimitzaciÃ³", type="primary")
    
    # BotÃ³ d'impressiÃ³ amb JS
    if col_print.button("ðŸ–¨ï¸ Imprimir resultats"):
    ...
    if run_btn:
        ...
        if df_consum is None or df_consum.empty:
            st.error("No s'han trobat dades de consum suficients per al perÃ­ode seleccionat.")
            return

        prices = calculate_tariffs(df_consum.index, p1, p2, p3)

        # Guardar dades compartides per la pestanya de coeficients variables
        gen_pav_model, gen_sn_model = load_solar_curves()
        st.session_state['cle_df_consum'] = df_consum
        st.session_state['cle_gen_pav'] = align_solar(gen_pav_model, df_consum.index)
        st.session_state['cle_gen_sn'] = align_solar(gen_sn_model, df_consum.index)
        st.session_state['cle_prices_arr'] = prices
        st.session_state['cle_p_exc_val'] = p_exc
        st.session_state['cle_min_kwp_val'] = min_kwp_val

        # OptimitzaciÃ³ SLSQP fixes (pestanya actual)
        ...
        st.success("OptimitzaciÃ³ finalitzada amb Ã¨xit!")

    # --- SUB-PESTANYES: FIXES | VARIABLES ---
    tab_fix, tab_var = st.tabs(["ðŸ“Œ Coeficients Fixes (SLSQP)", "ðŸ”€ Coeficients Variables (Horaris)"])

    with tab_var:
        render_cle_variable_optimizer()

    with tab_fix:
        # --- RENDERITZAT DE RESULTATS FIXES (DES DE L'ESTAT) ---
        if st.session_state.cle_results is not None:
            ...
            # --- 1. RESUM EXECUTIU ---
            st.markdown("### ðŸ“Š Resum Executiu Global")
            ...
            with col_k3:
                st.metric("ðŸ›¡ï¸ Cobertura Autoconsum", f"{pct_cobertura:.1f} %")
            with col_k4:
                st.metric("â™»ï¸ Aprofitament de Planta", f"{pct_aprofitament:.1f} %",
            ...
            fig_donut2.update_layout(title_text="DestÃ­ Energia Generada (kWh)", title_x=0.5,
                                     margin=dict(t=40, b=10, l=10, r=10), showlegend=False)
            fig_donut2.update_traces(textposition='inside', textinfo='percent+label',
            ...
            fig_donut3.update_layout(title_text="ProcedÃ¨ncia del Consum (kWh)", title_x=0.5,
                                     margin=dict(t=40, b=10, l=10, r=10), showlegend=False)
            fig_donut3.update_traces(textposition='inside', textinfo='percent+label',
                                     hovertemplate="%{label}<br>%{value:,.0f} kWh<extra></extra>")
            ...
            st.markdown("### ðŸ“‹ Resultats Detallats per Equipament")
            ...
            st.download_button(
                label="ðŸ“¥ Descarregar Resultats (CSV)",
            ...
            df_display['Coeficient Sala Nova'] = df_display['Coeficient Sala Nova'].apply(
                lambda x: f"{x:.6f}" if pd.notnull(x) else "")
            df_display['PotÃ¨ncia Sala Nova (kWp)'] = df_display['PotÃ¨ncia Sala Nova (kWp)'].apply(
                lambda x: f"{x:.2f} kWp" if pd.notnull(x) else "")
            df_display['Coeficient PavellÃ³'] = df_display['Coeficient PavellÃ³'].apply(
                lambda x: f"{x:.6f}")
            df_display['PotÃ¨ncia PavellÃ³ (kWp)'] = df_display['PotÃ¨ncia PavellÃ³ (kWp)'].apply(
                lambda x: f"{x:.2f} kWp" if pd.notnull(x) else "")
            df_display['Consum Anual (kWh)'] = df_display['Consum Anual (kWh)'].apply(
            ...
            df_display['Autoconsum (Sala Nova)'] = df_display['Autoconsum SN (kWh)'].apply(
                lambda x: f"{x:,.0f} kWh".replace(',', '.'))
            df_display['Autoconsum (PavellÃ³)'] = df_display['Autoconsum PAV (kWh)'].apply(
                lambda x: f"{x:,.0f} kWh".replace(',', '.'))
            df_display['Excedents Compensats'] = df_display['Excedents Compensats (kWh)'].apply(
                lambda x: f"{x:,.0f} kWh".replace(',', '.'))
            df_display['Excedents LlenÃ§ats'] = df_display['Excedents LlenÃ§ats a la xarxa (kWh)'].apply(
                lambda x: f"{x:,.0f} kWh".replace(',', '.'))
            df_display['Autoconsum Total'] = df_display['Autoconsum Total (kWh)'].apply(
                lambda x: f"{x:,.0f} kWh".replace(',', '.'))

            cols_order = [
                'CUPS', 'Nom',
                'Coeficient Sala Nova', 'PotÃ¨ncia Sala Nova (kWp)',
                'Coeficient PavellÃ³', 'PotÃ¨ncia PavellÃ³ (kWp)',
            ...
            # --- DASHBOARD MENSUAL ---
            st.markdown("### ðŸ“Š Dashboard AnalÃ­tic Mensual (Global Ajuntament)")

            monthly_aggs = []
            for m in range(1, 13):
                m_consum = sum([list(filter(lambda x: x['Mes'] == m, r['Mensual']))[0]['Consum']
                                for r in detailed_results])
                m_gen_sn = sum([list(filter(lambda x: x['Mes'] == m, r['Mensual']))[0]['GeneraciÃ³ Assignada SN']
                                for r in detailed_results])
                m_gen_pav = sum([list(filter(lambda x: x['Mes'] == m, r['Mensual']))[0]['GeneraciÃ³ Assignada PAV']
                                 for r in detailed_results])
                m_auto_sn = sum([list(filter(lambda x: x['Mes'] == m, r['Mensual']))[0]['Autoconsum SN']
                                 for r in detailed_results])
                m_auto_pav = sum([list(filter(lambda x: x['Mes'] == m, r['Mensual']))[0]['Autoconsum PAV']
                                  for r in detailed_results])
            ...
            fig1 = go.Figure()
            fig1.add_trace(go.Bar(x=df_months['Mes'], y=df_months['Import Net Xarxa (kWh)'],
                                  name='ImportaciÃ³ (Pagada)', marker_color='#a5d6a7'))
            fig1.add_trace(go.Bar(x=df_months['Mes'], y=df_months['Autoconsum SN (kWh)'],
                                  name='Autoconsum (Sala Nova)', marker_color='#fbc02d'))
            fig1.add_trace(go.Bar(x=df_months['Mes'], y=df_months['Autoconsum PAV (kWh)'],
                                  name='Autoconsum (PavellÃ³)', marker_color='#2e7d32'))
            fig1.update_layout(barmode='stack',
                               title="Estructura de l'Energia Mensual per a l'AdministraciÃ³",
                               xaxis_title='Mes', yaxis_title='kWh')
            st.plotly_chart(fig1, use_container_width=True)

            fig3 = go.Figure()
            exc_sn = df_months['GeneraciÃ³ SN (kWh)'] - df_months['Autoconsum SN (kWh)']
            exc_pav = df_months['GeneraciÃ³ PAV (kWh)'] - df_months['Autoconsum PAV (kWh)']
            fig3.add_trace(go.Bar(
                x=df_months['Mes'],
                y=df_months['Autoconsum SN (kWh)'] + df_months['Autoconsum PAV (kWh)'],
                name='Autoconsum Aprofitat', marker_color='#43a047'))
            fig3.add_trace(go.Bar(x=df_months['Mes'], y=exc_sn,
                                  name='Excedents No Aprofitats (Sala Nova)', marker_color='#fff59d'))
            fig3.add_trace(go.Bar(x=df_months['Mes'], y=exc_pav,
                                  name='Excedents No Aprofitats (PavellÃ³)', marker_color='#a5d6a7'))
            fig3.update_layout(barmode='stack',
                               title='DestÃ­ de la GeneraciÃ³ Solar: Aprofitament vs Excedents',
                               xaxis_title='Mes', yaxis_title='kWh')
