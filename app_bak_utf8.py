...
try:
    ...
    st.error("ÔØî No s'han trobat els secrets de Supabase. Configura .streamlit/secrets.toml")
...
CUPS_MAPPING = {
    ...
    "ES0031406053355001KM0F": "Enll Ve├»nat Nou",
    ...
    "ES0031406054364001YH0F": "Enll Sobr├ánigues",
...
def login_form():
    st.markdown("#### ­ƒöÉ Acc├®s al Sistema")
...
def sync_csv_to_db(df, mode="merge"):
    ...
    if mode == "replace":
        st.warning("ÔÜá´©Å Esborrant dades existents...")
        try:
    ...
    status_text.success("Sincronitzaci├│ Optimitzada Completada! ­ƒÜÇ")
...
def render_executive_report(df, lighting_cups, building_cups, all_cups, source_mode=""):
    st.header("­ƒôï Informe Executiu Anual")
    ...
    if len(years) < 2:
        ...
        target_year = col_y1.selectbox("Any d'An├álisi", years, index=default_target_idx)
        ...
        prev_year = col_y2.selectbox("Any de Comparaci├│", prev_year_options, index=def_prev_opt_idx)
    ...
    col_kpi3.markdown(f"**Var:** ­ƒÆí {light_var:+.1f}% | ­ƒÅó {build_var:+.1f}%")
    ...
    # 5. Top Movers
    st.subheader("R├ánquing de Variacions (Per CUPS)")
    ...
    with col_top1:
        st.markdown("##### ­ƒôë Top 5 Estalvis")
    ...
    with col_top2:
        st.markdown("##### ­ƒôê Top 5 Augments")
    ...
    st.markdown("---")
    st.subheader("ÔÿÇ´©Å Impacte Comunitat Energ├¿tica Local")
    ...
    if not self_cups:
        ...
        fig_comm = go.Figure()
        fig_comm.add_trace(go.Bar(x=df_comm["NomMes"], y=df_comm["Generaci├│ Solar"], name="Autoconsum", marker_color="gold"))
        ...
        # TABLE: Impact Data
        st.markdown("##### ­ƒôä Detall Mensual")
        table_rows = []
        for i, m_name in enumerate(df_comm["NomMes"]):
            val_self = df_comm["Generaci├│ Solar"][i]
        ...
        # Total Row
        tot_self = sum(df_comm["Generaci├│ Solar"])
    ...
    st.markdown("---")
    st.header("ÔÿÇ´©Å Resum Fotovoltaica (Sala Nova)")
    ...
    try:
        ...
        if resp_fv.data:
            ...
            if not df_fv_target.empty:
                ...
                c_pv3.metric("Estalvi Estimat (@0.15Ôé¼)", f"{total_gen * 0.15:,.2f} Ôé¼")
...
def main():
    ...
    # Sidebar
    st.sidebar.write(f"­ƒæñ {st.session_state.user.email}")
    ...
    if source_mode == "Pujar CSV Local (Processat)":
        ...
        if uploaded_file is not None:
             ...
             if df is not None:
                 st.write("---")
                 st.markdown("##### Ôÿü´©Å Configuraci├│ de C├árrega")
                 upload_mode = st.radio(
                     "Mode de Sincronitzaci├│", 
                     ["Fusionar / Actualitzar", "ÔÜá´©Å Esborrar Tot i Reempla├ºar"],
                 ...
                 if st.button("­ƒÆ¥ Guardar a Base de Dades"):
        ...
        st.sidebar.info("Puja fitxers originals (.csv) d'Edistribucion. S'agruparan i normalitzaran autom├áticament.")
        uploaded_files = st.sidebar.file_uploader("Pujar CSVs Originals", type=["csv"], accept_multiple_files=True)
        
        if uploaded_files:
            if st.sidebar.button("ÔÜÖ´©Å Processar i Previsualitzar"):
            ...
            if df is not None:
                 ...
                 if st.button("­ƒÆ¥ Guardar a Base de Dades (Edistribucion)"):
                     mode_code = "replace" if "Esborrar" in upload_mode else "merge"
                     sync_csv_to_db(df, mode=mode_code)
        ...
        st.sidebar.markdown("---")
        st.sidebar.subheader("­ƒöî Connexi├│ FusionSolar")
        ...
        if st.session_state.huawei_token:
            ...
            if stations:
                st.sidebar.markdown("---")
                # Debug Info Expander
                with st.sidebar.expander("Ôä╣´©Å Detalls T├¿cnics (Debug)"):
                     st.json(stations)
                ...
                if st.sidebar.button("­ƒôí Test Dades Temps Real"):
                     with st.spinner("Connectant a inversor..."):
                         ...
                         if rt_data:
                             ...
                             st.sidebar.error("No s'han rebut dades (Potser l'inversor est├á apagat/nit?)")
                ...
                if col_btn1.button("­ƒôà Importar Dades DI├ÇRIES (R├ápid)"):
                     if d_start > d_end:
                         st.error("ÔÜá´©Å La Data Inici no pot ser posterior a la Data Fi.")
                         st.stop()
                     
                     with st.spinner("Descarregant dades di├áries..."):
                         all_daily = []
                         
                         debug_container = st.expander("Logs de C├árrega (Diari)", expanded=True)
                         ...
                         if all_daily:
                             ...
                             if rows:
                                 ...
                                 pivot = pivot.fillna(0)
                                 df = pivot
                                 st.success(f"Dades Di├áries obtingudes: {len(df)} registres.")
                                 ...
                                 try:
                                     ...
                                     if fv_payload:
                                         ...
                                         st.toast(f"Guardats {len(fv_payload)} registres a FV_Sala_Nova", icon="­ƒÆ¥")
                                 except Exception as e:
                                     st.error(f"Error guardant a DB Sala Nova: {e}")
                                     
                             else: st.warning("No s'han trobat dades di├áries.")
                         else: st.error("Error obtenint dades di├áries.")

                # --- OPTION 2: HOURLY DATA (SLOW) ---
                if col_btn2.button("ÔÅ▒´©Å Importar Dades HOR├ÇRIES (Lent)"):
                    if d_start > d_end:
                         st.error("ÔÜá´©Å La Data Inici no pot ser posterior a la Data Fi.")
                         st.stop()
                    ...
                    debug_container = st.expander("Logs de C├árrega (Horari)", expanded=True)
                    
                    with st.spinner("Descarregant dades hor├áries... (Lent per evitar Errors 407)"):
                         # Stop Button Logic
                         ...
                         if days_range > 20:
                             st.error(f"ÔÜá´©Å Risc de Quota Di├ária: Has seleccionat {days_range} dies.")
                             st.error("El l├¡mit diari de Huawei ├®s de ~25 peticions. Si us plau, selecciona blocs de m├áxim 20 dies.")
                             st.stop()
                         ...
                         for i in range(days_range):
                             ...
                             if i < days_range - 1: # Don't wait after last day
                                 for s in range(65, 0, -1):
                                     progress_text.text(f"ÔÅ│ Esperant {s}s per l├¡mit API (Huawei 407)...")
                                     time.sleep(1)
                                 # time.sleep(65.0)
            ...
            if df is not None:
                 ...
                 if st.button("­ƒÆ¥ Guardar a Base de Dades (Integrar)"):
                     mode_code = "replace" if "Esborrar" in upload_mode else "merge"
                     sync_csv_to_db(df, mode=mode_code)
        ...
        with st.sidebar.expander("­ƒôà Rang de Dades (DB)", expanded=False):
             db_start = st.date_input("Inici", default_start)
             db_end = st.date_input("Fi", default_end)
             reload_btn = st.button("­ƒöä Actualitzar Dades")
        ...
        if df is None:
            st.info("La base de dades est├á buida o no s'han trobat dades en aquest rang.")
    ...
    if df is not None:
        ...
        if not (min_csv_date <= st.session_state.anchor_date <= max_csv_date):
             st.toast(f"Data ajustada al rang disponible: {min_csv_date} - {max_csv_date}", icon="ÔÜá´©Å")
        ...
        # --- Classification Step ---
        st.subheader("­ƒñû Classificaci├│ Autom├ática de CUPS")
        ...
        with st.expander("Veure Detall Classificaci├│", expanded=True):
            ...
            with col_class_1:
                st.markdown(f"**­ƒÆí Enllumenat ({len(lighting_cups)})**")
                st.dataframe(make_display_df(lighting_cups), hide_index=True)
            with col_class_2:
                st.markdown(f"**­ƒÅó Edificis ({len(building_cups)})**")
                st.dataframe(make_display_df(building_cups), hide_index=True)
            with col_class_3:
                st.markdown(f"**ÔÿÇ´©Å Autoconsum ({len(self_consumption_cups)})**")
        ...
        # --- Tabs ---
        tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["­ƒôè Panell Global", "­ƒôê Comparativa", "­ƒîâ Auditor Enllumenat", "­ƒñû AI Advisor", "ÔÿÇ´©Å Autoconsum", "ÔÿÇ´©Å FV Sala Nova"])
        ...
        # I will target up to "with tab1:" and rely on the fact that existing code follows.
            ...
            with col_nav1:
                 ...
                 mode = st.selectbox("Escala Temporal", ["Di├ária", "Setmanal", "Mensual", "Anual"], key="view_mode_t1_v2")
            
            with col_nav2:
                # Navigation Buttons
                col_b1, col_b2 = st.columns(2)
                if col_b1.button("Ô¼à´©Å", key="prev_t1"):
                    st.session_state.anchor_date = shift_date(mode, st.session_state.anchor_date, -1)
                if col_b2.button("Ô×í´©Å", key="next_t1"):
                    st.session_state.anchor_date = shift_date(mode, st.session_state.anchor_date, 1)
            ...
            with col_nav3:
                st.subheader(f"­ƒôà {start_d} - {end_d}")
        ...
        with tab2:
            ...
            with col_c1:
                mode_t2 = st.selectbox("Escala Temporal (Comparativa)", ["Di├ária", "Setmanal", "Mensual", "Anual"], key="mode_t2")
            with col_c2:
                col_cb1, col_cb2 = st.columns(2)
                if col_cb1.button("Ô¼à´©Å", key="prev_t2"):
                    st.session_state.anchor_date = shift_date(mode_t2, st.session_state.anchor_date, -1)
                if col_cb2.button("Ô×í´©Å", key="next_t2"):
                     st.session_state.anchor_date = shift_date(mode_t2, st.session_state.anchor_date, 1)
            
            start_d2, end_d2, freq_alias2 = get_date_range(mode_t2, st.session_state.anchor_date)
             
            with col_c3:
                 st.subheader(f"­ƒôà {start_d2} - {end_d2}")
            ...
            # Multiselect
            selection_series = col_sel.multiselect("Afegir/Treure S├¿ries al Gr├áfic", comp_options, key="series_multiselect", default=st.session_state.comp_series_sel)
            ...
            if selection_series:
                ...
                if not selected_years_comp:
                    ...
                    if selected_years_comp:
                        ...
                        st.caption("Visualitzaci├│ Gr├áfica del Resum Anual")
        ...
        with tab3:
            ...
            if not lighting_selected:
                ...
                # 1. Automated Scanner
                st.subheader("­ƒòÁ´©Å Detecci├│ d'Incid├¿ncies (Massiva)")
                ...
                if 'anomalies_found' in st.session_state and st.session_state['anomalies_found']:
                    ...
                    col_alert.error(f"ÔÜá´©Å S'han detectat {len(anomalies_df)} incid├¿ncies.")
                    col_total.metric("Energia Malbaratada (Est.)", f"{total_detected:.2f} kWh")
                    col_pct.metric("% sobre Total Enllumenat", f"{pct_excess:.2f} %")
                    
                    st.caption("Ôä╣´©Å Nota: Aquest c├álcul ├®s una estimaci├│ basada en dades hor├áries. La precisi├│ exacta de l'encesa/apagada dep├¿n de la resoluci├│ de les dades (hor├ária vs minutal).")
                ...
                st.markdown("---")
                st.subheader(f"­ƒöì Detall: {view_cups} el {view_date}")
                ...
                if not day_data.empty:
                    ...
                    fig_audit.update_layout(
                        title=f"Corba de C├árrega: {view_cups}",
                        xaxis=dict(
                            tickmode='linear',
        ...
        # --- Tab 4: AI Advisor ---
        with tab4:
            st.header("­ƒñû Assistent Virtual")
            
            with st.expander("Ôä╣´©Å Com funciona aquest assistent?"):
                st.markdown("""
                Aquest assistent aplica regles heur├¡stiques per detectar patrons an├▓mals o interessants:
                
                **1. Encesa Di├╝rna Sistem├ática (Enllumenat):**
            ...
            if st.button("Executar An├álisi Avan├ºada"):
            ...
            if st.session_state.show_advanced_advisor:
                ...
                with st.spinner("Analitzant patrons complexos..."):
                    ...
                    st.subheader("­ƒîÖ Consum Nocturn Elevat (Edificis)")
                    night_anomalies = []
                    ...
                    if night_anomalies:
                        ...
                        st.success("Ô£à No s'han detectat consums nocturns an├▓mals en edificis.")

                    st.markdown("---")

                    # --- 2. SCHOOL CALENDAR CHECK ---
                    st.subheader("­ƒÄô An├álisi Calendari Escolar")
                    school_keywords = ['escola', 'llar', 'col┬Àlegi', 'institut']  # Keywords
                    ...
                    if not selected_schools:
                        ...
                        if school_anomalies:
                            ...
                            sel_sch = st.selectbox("Veure Gr├áfic Dia Escolar An├▓mal", opt_sch)
                            ...
                            st.success("Ô£à Les escoles semblen apagar correctament en festius.")

                    st.markdown("---")

                    # --- 3. SEASONAL PATTERN ANALYSIS ---
                    st.subheader("ÔØä´©ÅÔÿÇ´©Å An├álisi Estacional (Hivern vs Estiu)")
                    ...
                    for c in building_cups:
                        ...
                        if avg_winter > (avg_summer * 1.5):
                            classification = "­ƒö┤ Hivernal (Calefacci├│?)"
                        elif avg_summer > (avg_winter * 1.5):
                            classification = "­ƒöÁ Estival (Aire Cond.?)"
                    ...
                    st.subheader("­ƒôè An├álisi de Patrons Temporals (Setmanal i Horari)")
                    ...
                    for c in building_cups:
                        ...
                        if ratio_weekend < 0.4:
                            patro_setmanal = "­ƒÅó Laborable (Tanca cap de setmana)"
                        elif ratio_weekend > 0.9:
                            patro_setmanal = "­ƒöä Continu (7 dies)"
                        else:
                            patro_setmanal = "Mixt"
                        ...
                        if 8 <= peak_hour <= 18:
                            patro_horari = f"ÔÿÇ´©Å Di├╝rn (Pic: {peak_hour}h)"
                        elif 19 <= peak_hour <= 23:
                            patro_horari = f"­ƒÆí Vespre (Pic: {peak_hour}h)"
                        else:
                            patro_horari = f"­ƒîÖ Nocturn (Pic: {peak_hour}h)"
        ...
        with tab5:
            st.header("ÔÿÇ´©Å Autoconsum i Comunitat Energ├¿tica")
            ...
            if not self_consumption_cups:
                ...
                # --- KPIs ---
                st.subheader("Balan├º Global de la Comunitat (Total Participants)")
                ...
                k4.metric("% Autoconsum Mitj├á", f"{comm_autarchy:.1f}%")
                ...
                with c1:
                    st.markdown("##### ÔÜí Evoluci├│ Mix Energ├¿tic (Comunitat)")
                    fig_bal = go.Figure()
                    ...
                    st.markdown("##### ­ƒôê Evoluci├│ % Autoconsum")
                ...
                with c2:
                    st.markdown("##### ­ƒì░ Quota Autoconsum per Participant")
        ...
        with tab6:
            st.header("ÔÿÇ´©Å Producci├│ Fotovoltaica - Sala Nova (Base de Dades)")
            ...
            try:
                ...
                if data_fv:
                    ...
                    k2.metric("├Ültima Lectura", f"{last_val:.2f} kWh", f"{last_date}")
                    k3.metric("Registres", len(df_fv))
                    
                    st.markdown("##### ­ƒôà Producci├│ Di├ária")
                    st.bar_chart(df_fv['potencia_fv'], color="#FFC300")
                    
                    # By Month
                    st.markdown("##### ­ƒùô´©Å Producci├│ Mensual")
